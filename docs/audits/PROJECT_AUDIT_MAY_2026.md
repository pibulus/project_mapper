...
- 2026-05-27: Local smoke initially appeared to have an invalid Gemini key because an inherited shell `GEMINI_API_KEY` overrode the repo `.env`; `/api/title` was hardened to return a fallback title plus warning instead of a hard 500.
...
- 2026-05-27: Implemented the highest-value parity slices from `FEATURE_PARITY_AUDIT`: saved export drafts, portable project backup/import, action item undo/mobile polish, selected-topic rename/remove, and a focused organic/readable graph layout toggle.
