...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		if (contentType.includes('multipart/form-data')) {
			...
			const conversationId = formData.get('conversationId')?.toString() || crypto.randomUUID();
		...
		if (contentType.includes('application/json')) {
			...
			console.log(`[API /process] Processing ${text.length} characters of text`);
		...
		let friendlyMessage = 'Something went wrong processing your input. Please try again.';
