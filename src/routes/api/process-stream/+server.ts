...
async function postUpdateToParty(roomId: string, update: any) {
	...
	try {
		...
		if (!response.ok) {
			console.error(`[API /process-stream] Failed to send update to party: ${response.statusText}`);
		}
	} catch (error) {
		console.error('[API /process-stream] Error sending update to party:', error);
	}
}
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		console.log(`[API /process-stream] Processing ${text.length} characters of text`);
