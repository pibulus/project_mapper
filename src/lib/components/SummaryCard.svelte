<script lang="ts">
  ...
  import { splitTextForTopic } from "$lib/utils/topicUtils";
  ...
  $: summaryParts = splitTextForTopic(summary, activeTopic);
</script>
  ...
  {:else}
    ...
    >
      {#each summaryParts as part}
        {#if part.match}
          <span class="topic-highlight">{part.text}</span>
        {:else}
          {part.text}
        {/if}
      {/each}
    ...
    {#if topicsCount > 0}
      ...
      >
        <p style="font-size: var(--pm-text-xs); color: var(--pm-brown);">
          <span style="font-weight: 700; color: var(--pm-pink);"
            >{topicsCount}</span
          > topics identified
        </p>
      </div>
