  ...
  >
    ...
    {#if isAdding}
      ...
      >
        ...
        ></textarea>
        <button class="btn-primary" on:click={handleSaveNewItem}
          >Save Item</button
        >
      </div>
    {/if}
    ...
    {#if sortedItems.length === 0 && !isAdding}
      <div class="empty-state-container">
        ...
        <p class="empty-state-text">
          {searchQuery ? "No matching tasks" : "No action items yet"}
        </p>
        {#if searchQuery}
        ...
        {:else}
          <p class="empty-state-hint">They’ll appear when the map is ready.</p>
        {/if}
      </div>
    {:else}
              ...
              <div style="flex: 1; min-width: 0;">
                {#if editingItemId === item.id}
                  <textarea
                    ...
                    on:keydown={(event) =>
                      handleDescriptionKeyDown(event, item.id)}
