...
currentProject.subscribe((project) => {
	if (project && project.actionItems) {
		...
		if (JSON.stringify(get(actionItems)) !== JSON.stringify(project.actionItems)) {
...
export function toggleItem(itemId: string) {
	...
	const updated = currentItems.map((i) =>
		i.id === itemId ? { ...i, status: i.status === 'completed' ? 'pending' : 'completed' } : i
	);
...
export function addItem(description: string) {
	...
	const newSortOrder =
		currentItems.length > 0 ? Math.min(...currentItems.map((i) => i.sort_order)) - 10 : 0;
...
export function reorderItems(newItems: ActionItem[]) {
	...
	const updatedWithSortOrder = newItems.map((item, index) => ({ ...item, sort_order: index }));
