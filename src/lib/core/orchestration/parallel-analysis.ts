...
export async function analyzeText(
	...
	const topicsPromise = aiService.extractTopics(text, existingNodes).then((topics) => {
	...
	const [topics, actionItems] = await Promise.all([topicsPromise, actionItemsPromise, summaryPromise]);

	// After topics and action items are extracted, check for status updates
	if (existingActionItems.length > 0) {
		aiService.checkActionItemStatus(text, existingActionItems).then((statusUpdates) => {
