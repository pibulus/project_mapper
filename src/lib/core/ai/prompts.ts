...
import type { ActionItem, EdgeInput, NodeInput } from "../types/index";
...
export const buildTopicExtractionPrompt = (
  ...
  const existingEdgesContext =
    existingEdges.length > 0
      ? `\n\nEXISTING RELATIONSHIPS (preserve these when still relevant):\n${existingEdges
          .map((edge) => {
            const source =
              labelById.get(edge.source_topic_id) || edge.source_topic_id;
            const target =
              labelById.get(edge.target_topic_id) || edge.target_topic_id;
